...
export default function TopReads({ blogTypeContent, filteredBlogData}: TopReadsProps) {
  ...
  return (
    <section className="w-[95%] mx-auto flex-col flex items- my-14 justify-center  gap-5">
        <div className="w-full flex justify-between items-center">
            ...
            { filteredBlogData.length < 3 ? (
                <div onClick={() => setHide((prev) => !prev)} className="text-mainColor cursor-pointer text-base hover:underline font-medium leading-[24px]">
                { !hide ? 'See all' : 'View top reads'}
                </div>
            ): ''}
        </div>
        <div className="grid  items-start gap-4 grid-cols-1 gap-y-10 text-customBlack w-full md:grid-cols-2 lg:grid-cols-3">
            {
                filteredBlogData.slice(0, hide ? -1 : 3).map((item, idx) => (
                    <div key={idx} className="flex flex-col items-start w-full justify-start gap-6">
                        ...
                        <div className="flex flex-col gap-4 items-start w-full">
                            <Link href={`/discover/${item.categories[0].slug.current}/${item.slug.current}`} className=" hover:text-mainColor transition-colors duration-150 font-semibold text-[24px] leading-[24px]">{item.title}</Link>
