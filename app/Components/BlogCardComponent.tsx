...
export default function BlogCardComponent({ blogTypeContent, filteredBlogData}: BlogCardComponentProps) {
    ...
    <section className="w-[95%] mx-auto flex-col flex items- my-14 justify-center  gap-5">
        <div className="w-full flex justify-between items-center">
            <h3 className="leading-[24px] text-[36px] font-semibold text-customBlack">{blogTypeContent}</h3>
                <Link href={`/discover/`} className="text-mainColor cursor-pointer text-[24px] hover:underline font-medium leading-[24px]">
                    View more
                </Link>
        </div>
        <div className="grid  items-start gap-4 grid-cols-1 gap-y-10 text-customBlack w-full md:grid-cols-2 lg:grid-cols-3">
            {
                filteredBlogData.slice(0, 4).map((item, idx) => (
                    <div key={idx} className="flex flex-col items-start w-full justify-start gap-6">
                        ...
                        <div className="flex flex-col gap-4 items-start w-full">
                            ...
                            <p className=" text-base leading-[24px] line-clamp-3 font-semibold text-[#545454]">
                               {item.description}
                            </p>
